...
public class WarehouseController {
    ...
    public WarehousePresentService warehousePresentService;

    @Autowired
    public WarehouseController(WarehouseService warehouseService,WarehousePresentService warehousePresentService) {
...
//    }
    ...
    @PostMapping("/present")
    @Authorized(roles = {Role.ADMIN, Role.GM, Role.INVENTORY_MANAGER})
    public Response warehousePresent(UserVO userVO,@RequestBody WarehousePresentSheetVO warehousePresentSheetVO){
        log.info(warehousePresentSheetVO.toString());
        warehousePresentService.makeSheet(userVO,warehousePresentSheetVO);
        return Response.buildSuccess();
    }
    ...
    public Response getWarehousePresent(){
        return Response.buildSuccess(warehousePresentService.getSheetByState(null));
    }

    @GetMapping("/approvePresent")
    @Authorized(roles = {Role.ADMIN, Role.GM, Role.INVENTORY_MANAGER})
    public Response warehousePresentSheetApprove(@RequestParam(value = "sheetId") String sheetId,
        ...
        if (state.equals(WarehousePresentSheetState.FAILURE) || state.equals(WarehousePresentSheetState.SUCCESS)) {
            warehousePresentService.approval(sheetId,state);
        }
        else {
            throw new MyServiceException("C00001", "越权访问！");
        }
