...
import com.nju.edu.erp.service.Sheet.PurchaseReturnsService;
...
public class PurchaseReturnsServiceImpl implements PurchaseReturnsService {
    ...
    @Override
    @Transactional
    public void makeSheet(UserVO userVO, PurchaseReturnsSheetVO purchaseReturnsSheetVO) {
     ...
     */
    @Override
    public List<PurchaseReturnsSheetVO> getSheetByState(PurchaseReturnsSheetState state) {
    ...
    @Override
    public List<PurchaseReturnsSheetVO> findByFilter(SheetFilterVO filterVO) {
        Assert.isTrue(filterVO.getType()== SheetType.PURCHASE_RETURN||filterVO.getType()==null,"过滤类型需为进货退货单");
        SheetFilterPO filterPO=new SheetFilterPO();
        BeanUtils.copyProperties(filterVO,filterPO);
        List<PurchaseReturnsSheetPO> sheetPOS = purchaseReturnsSheetDao.findByFilter(filterPO);
        return sheetPOS.stream().map(this::transformPOtoVO).collect(Collectors.toList());
    }

    private PurchaseReturnsSheetVO transformPOtoVO(PurchaseReturnsSheetPO po){
        PurchaseReturnsSheetVO vo=new PurchaseReturnsSheetVO();
        BeanUtils.copyProperties(po,vo);
