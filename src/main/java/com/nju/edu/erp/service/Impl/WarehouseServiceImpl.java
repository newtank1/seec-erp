...
//            warehouseDao.deductQuantity(warehousePO);
    ...
    public List<WarehouseIODetailPO> getWarehouseIODetailByTime(String beginDateStr,String endDateStr) {
         ...
         *
         */
        if (beginDateStr.compareTo(endDateStr) > 0) return null;

        List<WarehouseIODetailPO> res = new ArrayList<>();
        List<WarehouseInputSheetPO> warehouseInputSheetPOS = warehouseInputSheetDao.getAllSheets();
        List<WarehouseOutputSheetPO> warehouseOutputSheetPOS = warehouseOutputSheetDao.getAllSheets();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

        for (WarehouseInputSheetPO sheet : warehouseInputSheetPOS) {
            Date time = sheet.getCreateTime();
            String date = simpleDateFormat.format(time);

            if (date.compareTo(beginDateStr) < 0 || date.compareTo(endDateStr) > 0) continue;

            List<WarehouseInputSheetContentPO> productsList = warehouseInputSheetDao
                    .getAllContentById(sheet.getId());
            for (WarehouseInputSheetContentPO product : productsList) {
                WarehouseIODetailPO detailPO = WarehouseIODetailPO.builder()
                        .type("input")
                        .sheetId(sheet.getId())
                        .quantity(product.getQuantity())
                        .unitPrice(product.getPurchasePrice())
                        .createTime(sheet.getCreateTime())
                        .build();
        ...
        for (WarehouseOutputSheetPO sheet : warehouseOutputSheetPOS) {
            Date time = sheet.getCreateTime();
            String date = simpleDateFormat.format(time);

            if (date.compareTo(beginDateStr) < 0 || date.compareTo(endDateStr) > 0) continue;

            List<WarehouseOutputSheetContentPO> productsList = warehouseOutputSheetDao
                    .getAllContentById(sheet.getId());
            for (WarehouseOutputSheetContentPO product : productsList) {
                WarehouseIODetailPO detailPO = WarehouseIODetailPO.builder()
                        .type("input")
    ...
    public int getWarehouseInputProductQuantityByTime(String beginDateStr,String endDateStr){
        ...
        if (beginDateStr.compareTo(endDateStr) > 0) return 0;

        List<WarehouseInputSheetPO> sheetsList = warehouseInputSheetDao.getAllSheets();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        for (WarehouseInputSheetPO sheet : sheetsList) {
            Date time = sheet.getCreateTime();
            String date = simpleDateFormat.format(time);
    ...
    public int getWarehouseOutProductQuantityByTime(String beginDateStr,String endDateStr){
        ...
        List<WarehouseOutputSheetPO> sheetsList = warehouseOutputSheetDao.getAllSheets();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        for (WarehouseOutputSheetPO sheet : sheetsList) {
            Date time = sheet.getCreateTime();
            String date = simpleDateFormat.format(time);
